...
describe Money::Bank::OpenExchangeRatesBank do
  ...
  describe 'update_rates with bid and ask' do
    before do
      subject.app_id = TEST_APP_ID
      subject.cache = oer_latest_path
      # Mock the API response to include bid and ask rates
      subject.stubs(:api_response).returns(File.read(data_file('latest.json')))
      subject.update_rates
    end

    it 'should store bid rates correctly' do
      expected_bid = { 'USD' => 1.0, 'EUR' => 0.89 } # Replace with actual expected values
      assert_equal(expected_bid, subject.get_bid('USD', 'EUR'))
    end

    it 'should store ask rates correctly' do
      expected_ask = { 'USD' => 1.0, 'EUR' => 0.91 } # Replace with actual expected values
      assert_equal(expected_ask, subject.get_ask('USD', 'EUR'))
    end
  end
  describe 'retrieve bid and ask rates' do
  before do
    subject.update_rates
  end

  it 'should retrieve bid rate for USD to EUR' do
  money = Money.new(100, 'USD')  # $1.00
    expected_bid = Money.new(89, 'EUR')  # Expected bid conversion
    assert_equal(expected_bid, subject.get_bid('USD', 'EUR'))
  end

  it 'should retrieve ask rate for USD to EUR' do
    money = Money.new(100, 'USD')  # $1.00
    expected_ask = Money.new(91, 'EUR')  # Expected ask conversion
    assert_equal(expected_ask, subject.get_ask('USD', 'EUR'))
  end
end
describe 'error handling for bid and ask rates' do
  it 'should handle missing bid rate gracefully' do
    subject.stubs(:api_response).returns(File.read(data_file('incomplete_bid_ask.json')))
    subject.update_rates
    assert_nil(subject.get_bid('USD', 'EUR'))
  end

  it 'should handle missing ask rate gracefully' do
    subject.stubs(:api_response).returns(File.read(data_file('incomplete_bid_ask.json')))
    subject.update_rates
    assert_nil(subject.get_ask('USD', 'EUR'))
  end
end
